...
public class TestCases {
    ...
    public TestCases(LocationSensorApp lsapp) {
        mLSApp = lsapp;
        mDbHelper = LocationDatabaseHelper.getInstance(mLSApp);
    ...
    public void testCheckinData() {
        mLSApp.getLSMan().mStore.checkinUponLeavingLocation(null);
    }
    ...
    private void dumpCursor() {
        ...
        Cursor c = mLSApp.getContentResolver().query(LocationSensorProvider.LOCTIME_CONTENT_URI, new String[] { "name", "accuracy", "bestaccuracy", "accuname"}, null, null, null);
        if (c.moveToFirst()) {
            do {
    ...
    private void testWifiMatch() {
        mLSApp.getLSMan().mDetection.matchBeacons();
    }
    ...
    private void testConsolidateUri() {
        long id = 36;
        String CONSOLIDATE_URI = "content://com.motorola.locationsensor/loctime/consolidate";
        String where = "( _ID = " + id + " )";
        String order = " _ID DESC";
        Cursor c = mLSApp.getContentResolver().query(Uri.parse(CONSOLIDATE_URI), null, where, null, order);
        try {
    ...
    private void testContentProviderUri() {
        long id = 100;
        String LOCTIME_URI = "content://com.motorola.locationsensor/loctime";
        String where = "( _ID <= " + id + " )";
        String order = " _ID DESC";
        Cursor c = mLSApp.getContentResolver().query(Uri.parse(LOCTIME_URI), null, where, null, order);
        try {
            if (c != null && c.moveToFirst()) {
                do {
                    Tuple t = LocationDatabase.LocTimeTable.toTuple(c);
